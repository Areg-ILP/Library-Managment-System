...
void LibUpdate::DefineBooks()
{
	...
	book_info copy;
	while(rlib >> copy.name_author >> copy.name_book)	
...
void LibUpdate::DelBook(book_info B)
{
	...
	{
		if(B.name_author == this->books[i].name_author && B.name_book == this->books[i].name_book)
		{
			advance(it,i);
			this->books.erase(it);
		}
	...
	for(size_t i = 0; i < this->books.size(); i++)
		clearF << this->books[i].name_author << " " << this->books.at(i).name_book << "\n";
...
void User::DefineBooks()
{
	...
	book_info copy;
	int id;
	while(rub >> id >> copy.name_author >> copy.name_book)
	{
		if(this->GetId() == id)
			this->books.push_back(copy);
	}	
...
void User::AddBook(book_info B)
{
	...
	rmb << this->GetId() << " " << B.name_author + " " + B.name_book << "\n";
...
void User::DelBook(book_info B)
{
	...
	for(size_t i = 0; i < this->books.size(); i++)
		clearUB << this->GetId() << this->books[i].name_author << " " << this->books.at(i).name_book << "\n";
